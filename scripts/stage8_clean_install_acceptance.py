...
def main() -> int:
    ...
    try:
        ...
        run([str(python), "-m", "pip", "install", str(wheel)], cwd=external)
        reinstall_verified = run([str(phase), "--version"], cwd=external).stdout.strip() == "phase 1.1.0-rc.1"
        ...
        summary["success"] = all([
            ...
            imported == "1.1.0-rc.1", summary["help_discovery"], doctor["success"], summary["cli_parse_rejections"],
