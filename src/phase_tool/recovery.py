...
def load_context(application, evidence_root: Path, run_id: str, target_root: Path,
    ...
    if binding['id'] not in {'file_create.v1','file_create.v2','bundle_create.v1'}:
...
def verify_without_final_receipt(application, run: Path, target: Path, intent: dict, plan: dict) -> None:
    ...
    _reject_existing_links(pre_path)
    pre=parse_json_bytes(read_bounded_file(pre_path,REQUEST_LIMIT))
    if not isinstance(pre,list) or not pre or any(item['blocking'] and item['phase']!='post_operation' and item['status']!='pass' for item in pre):
    ...
    if plan['mechanism']['id']=='mechanism.bundle_create_v1':
