...
def test_product_cli_reports_version_health_and_registry_contracts() -> None:
    ...
    assert version.stdout.strip() == "phase 1.1.0-rc.5"
    ...
    assert health["version"] == "1.1.0-rc.5"
...
def test_publication_metadata_documentation_and_linux_ci_are_complete() -> None:
    ...
    assert project["version"] == "1.1.0rc5"
...
def test_wheel_sdist_clean_install_uninstall_reinstall_acceptance() -> None:
    ...
    assert summary["import_version"] == "1.1.0-rc.5"
    assert summary["help_discovery"] is True
    assert summary["version"] == "phase 1.1.0-rc.5"
